...
def clean_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['name'] = (df.genus + ' ' + df.species).astype('str')
    df['name'] = df.name.apply(lambda value: value.strip())
    ...
    df['name_authorship'] = df.name_authorship.apply(lambda val: val.replace('(', ''))
    df['name_authorship'] = df.name_authorship.apply(lambda val: val.replace(')', ''))
